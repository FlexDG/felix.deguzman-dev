import{L as e,O as t,St as n,Y as r,at as i,bt as a,dt as o,ft as s,it as c,j as l,k as u,l as d,mt as f,pt as p,r as m,u as h,vt as g,xt as _,yt as v}from"./index-Cr_RREAy.js";var y=n(_(),1),b=`
  varying vec2 vUv;
...
`,S=v(),C={desktop:[.49,.62],mobile:[.43,.55]},w=[.8902,.8706,.9882],T=[.251,.169,.314],E=1.75,D=32e5;function O(e,t){let n=Math.min(window.devicePixelRatio||1,E);return Math.min(n,Math.sqrt(D/Math.max(1,e*t)))}function k(){if(typeof document>`u`)return!1;try{let e=document.createElement(`canvas`),t=e.getContext(`webgl2`)||e.getContext(`webgl`);return t?.getExtension(`WEBGL_lose_context`)?.loseContext(),!!t}catch{return!1}}var A=k();function j(){return{uTex:{value:null},uResolution:{value:new s(1,1)},uRect:{value:new f(0,0,1,1)},uFocalUv:{value:new s(.49,.38)},uZoom:{value:1},uCentering:{value:0},uBlur:{value:0},uAberration:{value:0},uCorrode:{value:0},uEdge:{value:.05},uWarp:{value:0},uRim:{value:0},uWhite:{value:0},uVignette:{value:0},uGrain:{value:0},uCover:{value:0},uTime:{value:0},uBg:{value:new p(...w)}}}function M({apiRef:n,onReady:s}){let f=(0,y.useRef)(null),p=(0,y.useRef)(null),[_,v]=(0,y.useState)(A?`webgl`:`css`);return(0,y.useEffect)(()=>{if(_!==`webgl`)return;let p=f.current;if(!p)return;let y=p.closest(`[data-hero-pane]`),S=document.querySelector(`[data-hero-img]`);if(!y||!S)return;let E=document.createElement(`canvas`);E.setAttribute(`aria-hidden`,`true`),E.style.cssText=`display:block;width:100%;height:100%`,p.appendChild(E);let D;try{D=new m({canvas:E,alpha:!0,premultipliedAlpha:!0,antialias:!1,powerPreference:`high-performance`})}catch{D=null}if(!D||!D.getContext()){D?.dispose(),E.remove(),queueMicrotask(()=>v(`css`));return}D.setClearColor(0,0),D.outputColorSpace=l;let k=j(),A=new c,M=new d,N=new i({uniforms:k,vertexShader:b,fragmentShader:x,transparent:!0,blending:0,depthTest:!1,depthWrite:!1}),P=new r(2,2);A.add(new e(P,N));let F=()=>{!k.uTex.value||D.getContext().isContextLost()||D.render(A,M)},I=new o,L=null,R=!1,z=e=>{!e||e===L||(L=e,I.load(e,e=>{if(R){e.dispose();return}e.colorSpace=``,e.premultiplyAlpha=!0,e.generateMipmaps=!0,e.minFilter=u,e.magFilter=t,e.wrapS=h,e.wrapT=h,e.anisotropy=D.capabilities.getMaxAnisotropy(),e.needsUpdate=!0,k.uTex.value?.dispose(),k.uTex.value=e,F()}))},B=()=>{let e=y.getBoundingClientRect(),t=S.getBoundingClientRect(),n=Math.max(1,Math.round(e.width)),r=Math.max(1,Math.round(e.height));D.setPixelRatio(O(n,r)),D.setSize(n,r,!1),k.uResolution.value.set(n,r),k.uRect.value.set(t.left-e.left,e.bottom-t.bottom,Math.max(1,t.width),Math.max(1,t.height));let i=S.currentSrc||S.src,a=/mobile_hero/.test(i)?C.mobile:C.desktop;k.uFocalUv.value.set(a[0],1-a[1]),z(i),F()};B();let V=new ResizeObserver(B);V.observe(y),V.observe(S);let H=g(B),U=!1;document.fonts?.ready.then(()=>{!U&&!R&&B()});let W=e=>{e.preventDefault(),document.documentElement.setAttribute(`data-portal-lost`,``)},G=()=>{document.documentElement.removeAttribute(`data-portal-lost`),L=null,B()};E.addEventListener(`webglcontextlost`,W),E.addEventListener(`webglcontextrestored`,G);let K=!1,q=(e,t)=>{!K||!k.uTex.value||D.getContext().isContextLost()||(k.uTime.value+=Math.min(t,50)/1e3,D.render(A,M))};return a.ticker.add(q),n.current={mode:`webgl`,uniforms:k,room:{from:w,to:T},layer:p,setActive:e=>{K=e,e&&B()}},s?.(`webgl`),()=>{R=!0,U=!0,a.ticker.remove(q),V.disconnect(),H(),E.removeEventListener(`webglcontextlost`,W),E.removeEventListener(`webglcontextrestored`,G),document.documentElement.removeAttribute(`data-portal-lost`),k.uTex.value?.dispose(),P.dispose(),N.dispose(),D.dispose(),D.forceContextLoss(),E.remove(),n.current=null}},[_,n,s]),(0,y.useEffect)(()=>{if(_===`css`)return n.current={mode:`css`,el:p.current},s?.(`css`),()=>{n.current=null}},[_,n,s]),_===`css`?(0,S.jsx)(`div`,{ref:p,"aria-hidden":`true`,className:`pointer-events-none absolute inset-0 z-[2] opacity-0`,style:{background:`radial-gradient(circle at var(--hero-portal-fx, 50%) var(--hero-portal-fy, 60%), #fff 0%, #fff 38%, rgb(255 255 255 / 0) 72%)`}}):(0,S.jsx)(`div`,{ref:f,"data-portal-layer":!0,"aria-hidden":`true`,className:`pointer-events-none invisible absolute inset-0 z-[2] opacity-0`})}export{M as default};